...
''' 추가 : 은경 2018.02.
index data in form of naive texts instea of c-struct binary
add 'wstring' for checking word itself
add unit test to the very last line
'''
from namuip.document import Storage #python3
from namuip.common import * #python3
#unit test
'''
...
class Indexer:
    ...
    def build(storageFileName, outputFileName):
        ...
        print(('\n구문 구조 정보 정렬 Temporarily Not'))
        '''for key, syntaxInfoList in indexMap.items(): #jek
    ...
    def __sortSyntaxInfoList(syntaxInfoList):
        ...
        syntaxInfoArray = []
        for i in range(syntaxInfoList.length()):
        ...
        #--syntaxInfoArray.sort() #jek. in python3, TypeError: '<' not supported between instances of 'SyntaxInfo' and 'SyntaxInfo'
    ...
    def __writeToFile(indexMap, outputFileName):
        ...
        print(('색인 정보를 파일로 저장합니다...'))
        wf = open(outputFileName, 'w')
        totKeyLength = 0
        totSyntaxInfoCount = 0
        keyLengthArray = []
        keyCharArray = [] #keys of syn nodes
        syntaxInfoCountArray = [] # num of syn nodes
        syntaxInfoArray = [] # syn node
        # 정보를 위 변수와 배열에 담는다.
        for key, syntaxInfoList in indexMap.items(): #jek
            keyLength = len(key)
            totKeyLength += keyLength
            keyLengthArray.append(str(keyLength))
            keyCharArray.extend(key)
            totSyntaxInfoCount += syntaxInfoList.length()
            syntaxInfoCountArray.append(str(syntaxInfoList.length()))
            for i in range(syntaxInfoList.length()):
                a_tuple = (syntaxInfoList.sentenceIds[i],
                syntaxInfoList.startIdxs[i],
                syntaxInfoList.endIdxs[i],
                syntaxInfoList.openPIdxs[i],
                syntaxInfoList.closePIdxs[i])
                syntaxInfoArray.append(str(a_tuple))
        ...
        wf.write('%s\n' %(','.join(keyLengthArray)))
        wf.write('%s\n' %(''.join(keyCharArray)))
        wf.write('%s\n' %(','.join(syntaxInfoCountArray)))
        wf.write('%s\n' %('|'.join(syntaxInfoArray)))
        wf.close()
        print('len of indexmap:%d' %len(indexMap))
        print('len of keyCharArray:%d' %len(keyCharArray))
        print('len of syntaxInfoCountArray:%d' %len(syntaxInfoCountArray))
        print('len of syntaxInfoArray:%d' %len(syntaxInfoArray))
        print('totsyntaxInfoCount:%d' %totSyntaxInfoCount)
        return

    @staticmethod
    def __writeToFile0(indexMap, outputFileName):
        """ 색인을 파일로 저장한다.
        Searcher의 __loadIndex 함수와 호환되어야 한다. """
        print(('색인 정보를 파일로 저장중'))
        ...
        keyCharArray = array.array('u') #jek
        # 각 key마다 구문정보 개수
    ...
    def __buildParseTree(egSentence):
        ...
        for lineNum in range(len(egLines)):
            ...
            while(i+1 < len(egLine)):
                i = i + 1
                # if '/' follows after '(' . it's symbol tag, not structural parenthesis: '(' 나 ')'는 /SS tag가 붙어있다.
                if egLine[i] == '(' and i+1 < len(egLine) and egLine[i+1] != '/':
                    if curNode.left == None: # if structural parenthesis,
                    ...
                    tagEndIdx2 = egLine.find(';', i+1) #??? which cases
                    ...
                    if tag not in tag2symbol:
                        # 의미 없는 태그 문자로 대체
                        print('ztag:', tag, file=sys.stderr) #, egLine #XXX
                        curNode.tag = 'z'
                    else:
                ...
                #print(__name__, lineNum, openPIdx, closePIdx, egLine)
                if curNode == rootNode:
                    break
        return rootNode
...
if __name__ == "__main__":
    try :
        ...
        import pdb
        pdb.set_trace()
        test.build('storage.data', 'index.data')
